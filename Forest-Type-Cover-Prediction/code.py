...
# Code Starts Here
# create a subset of a dataframe with only first 10 numbers 
...
#Scales are not the same for all variables. Hnece, rescaling and standardization is required
X_train, X_test, Y_train, Y_test = cross_validation.train_test_split(X,Y,test_size=0.2,random_state=0)

#Standardized 
scaler = StandardScaler()

#CApply transform only for continous data 
X_train_temp = scaler.fit_transform(X_train.iloc[:,:10])
X_test_temp = scaler.transform(X_test.iloc[:,:10])

#Concatenate scaled continous data and categorical 
